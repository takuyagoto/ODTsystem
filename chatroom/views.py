...
def load_all_user(request):
    ...
    paginaobj = Paginator(user_list, 3)  # 1为每页显示的,实现分页功能
    ...
    return HttpResponse(json.dumps([return_user, page_html]))#对简单数据类型进行编码
...
def send_msg(request):
    ...
    if request.method == "POST":
        ...
        data['from_user'] = request.user.loginuser.id
        data['send_date'] = time.strftime("%X", time.localtime())  #格式化时间字符串
        # print(data)
        analysis_msg(data)
        return HttpResponse("OK")
...
def analysis_msg(data):
    ...
    if send_to_type == "user":
        ...
        for user in user_list:
            user_id = int(user['id'])
            if user_id != data['from_user']:   #不用自己给自己送信
...
def store_msg(userid, data):
    ...
    if not userid in GLOBAL_QUEUE.keys():  #返回global_queue中有可用的键
